...
/// Compute pass that assigns splats to tiles based on their screen-space bounding boxes
public struct TileBinningComputePass: Element {
    // MARK: - Properties
    ...
    public init(
        ...
        self.computeKernel = try shaderLibrary.function(named: "tile_binning", type: ComputeKernel.self)
    }
    ...
    public var body: some Element {
        get throws {
            // Clear tile counters before binning
            try BlitPass {
                Blit { encoder in
                    encoder.fill(
                    ...
                    // Note: We don't need to clear tileSplatIndices since we only read
                    // entries up to the counter value
                }
            }

            // Run binning kernel - one thread per splat
            try ComputePass(label: "Tile Binning") {
